...
Generates a concise 2–3 sentence executive newsroom briefing synthesising key developments across retrieved breaking items.

* **Parameters:**
  * `prompt` *(string)*: Editorial focus or question.
  * `scoop_ids` *(array of strings)*: Target scoops to summarise.

### 4. `make_offer` *(Enterprise Only)*
Enables authorised AI agents to programmatically lock escrow funds and submit commercial licence offers.

* **Parameters:**
  * `scoop_id` *(string)*: The scoop to acquire.
  * `amount` *(number)*: Offer price in GBP (£5.00 to £1,000,000.00).
  * `expiry_minutes` *(number)*: Expiry countdown window (`3`, `5`, `10`, `30`, or `60`).
...
> *"Check Extra Scoop for verified breaking footage of the transport protest in central London over the last 24 hours. Summarise the key angles and let me know if exclusive broadcast rights are available."*
